...
def winwing_fcu_set_led(dev, led, brightness):
...
for d in devlist:
    ...
    if len(device) != 0:
        ...
        #print(f"dev: {device}")
        device=device[0]
        print(f"found serial:{h.get_serial_number_string()}, release_number:{device['release_number']}")
...
winwing_fcu_set_led(device, Leds.BACKLIGHT, 70)
winwing_fcu_set_led(device, Leds.SCREEN_BACKLIGHT, 200)

while True:
    buf_in = [None] * 7
    #num_bytes = endpoint_in.read(0x81, 7)
    #print(num_bytes)
    winwing_fcu_set_led(device, Leds.AP1_GREEN, 1)
    winwing_fcu_set_led(device, Leds.AP2_GREEN, 0)
    winwing_fcu_lcd_set(device, speed, heading, alt, vs, 0x0)
    speed = speed + 1
    #heading = heading + 3
    time.sleep(0.5)
    winwing_fcu_set_led(device, Leds.AP1_GREEN, 0)
    winwing_fcu_set_led(device, Leds.AP2_GREEN, 1)
    winwing_fcu_lcd_set(device, speed, heading, alt, vs, 0xff)
    time.sleep(0.5)
