...
datarefs = [
    ...
    ("sim/cockpit2/autopilot/vnav_speed_window_open", 2),
    ("laminar/A333/autopilot/vvi_fpa_window_open", 2),
    ("laminar/A333/autopilot/hdg_window_open", 2),
    ("sim/cockpit2/autopilot/trk_fpa", 2),
    ("laminar/A333/annun/autopilot/alt_mode", 2), #annun means annunciator 
    ("laminar/A333/autopilot/fpa_increase",2),
    ("laminar/A333/autopilot/fpa_decrease",2),
]
...
def create_button_A333_fcu():
    ...
    buttonlist.append(Button(15, "HDG PUSH", "laminar/A333/autopilot/heading_knob_push", DREF_TYPE.CMD, BUTTON.TOGGLE))
    buttonlist.append(Button(16, "HDG PULL", "laminar/A333/autopilot/heading_knob_pull", DREF_TYPE.CMD, BUTTON.TOGGLE))
...
def fix_button(id, dataref):
   for b in buttonlist:
       if b.id != id:
           continue
       if b.dataref == dataref:
           break
       print(f'fix dataref {b.dataref} to {dataref}')
       b.dataref = dataref
       break
...
def set_datacache(values):
    ...
    if new == True or usb_retry == True:
        ...
        hdg = datacache['sim/cockpit2/autopilot/trk_fpa']
        if vs < 0:
            vs = abs(vs)
        ...
        if datacache['sim/cockpit2/autopilot/vnav_speed_window_open'] == 0:
            speed = '---'
        if datacache['laminar/A333/autopilot/hdg_window_open'] == 0:
            heading = '---'
        if datacache['laminar/A333/autopilot/vvi_fpa_window_open'] == 0:
            vs = '----'
            flags['vs_vert'].value = False
        elif not hdg:
            ...
            vs = vs.ljust(4, '#')
            #print(f"vs: {v}")
            fix_button(21, "sim/autopilot/vertical_speed_down")
            fix_button(22, "sim/autopilot/vertical_speed_up")
        else:
            vs = string_fix_length(int(vs / 100), 2)
            vs = vs.ljust(4, ' ')
            flags['fpa_comma'].value = True
            fix_button(21, "laminar/A333/autopilot/fpa_decrease")
            fix_button(22, "laminar/A333/autopilot/fpa_increase")
        
        flags['spd_managed'].value = not datacache['sim/cockpit2/autopilot/vnav_speed_window_open']
        flags['hdg_managed'].value = not datacache['laminar/A333/autopilot/hdg_window_open']
        flags['alt_managed'].value = not datacache['laminar/A333/autopilot/vvi_fpa_window_open']
        ...
        if True:
            try: # dataref may not be received already, even when connected
                exped_led_state_desired = datacache['aminar/A333/annun/autopilot/alt_mode'] > 0
            except:
